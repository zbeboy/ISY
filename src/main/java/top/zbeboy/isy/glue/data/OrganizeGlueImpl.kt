    ...
    override fun sortCondition(dataTablesUtils: DataTablesUtils<OrganizeBean>, nativeSearchQueryBuilder: NativeSearchQueryBuilder): NativeSearchQueryBuilder? {
        ...
        if (StringUtils.hasLength(orderColumnName)) {
            if ("organize_id".equals(orderColumnName, ignoreCase = true)) {
                if (isAsc) {
                    nativeSearchQueryBuilder.withSort(SortBuilders.fieldSort("organizeId.keyword").order(SortOrder.ASC).unmappedType("string"))
                } else {
                    nativeSearchQueryBuilder.withSort(SortBuilders.fieldSort("organizeId.keyword").order(SortOrder.DESC).unmappedType("string"))
                }
            }
