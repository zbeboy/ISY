...
require(["jquery", "handlebars", "constants", "nav_active", "messenger", "jquery.address", "bootstrap-maxlength", "jquery.showLoading"],
    function ($, Handlebars, constants, nav_active) {
        ...
        var ajax_url = {
            ...
            save: '/web/data/schoolroom/save',
            valid: '/web/data/schoolroom/save/valid',
        ...
        $(paramId.buildingCode).blur(function () {
            ...
            if (buildingCode.length <= 0 || buildingCode.length > 200) {
                validErrorDom(validId.buildingCode, errorMsgId.buildingCode, '教室名10个字符以内');
            } else {
                // 教室名是否重复
                Messenger().run({
                    errorMessage: '请求失败'
